<template>
  <div class="survey-form">
    <el-card shadow="hover" class="form-card">
      <el-form label-width="120px" class="form-body">
        <!-- 选择工程 -->
    ...
    <el-card shadow="hover" class="history-card">
      <template #header>
        <div class="history-header">
          <span class="section-title">项目勘察单生成历史</span>
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(
      `确定要删除 "${row.project_name || '该工程'}" 的项目勘察单记录吗？`,
      '删除确认',
      {
...
// 生成勘察单（不传任何选择项，后端自动从工程关联数据组合）
const handleGenerate = async () => {
  generating.value = true
  try {
    const res = await generateSurvey({
    ...
    ElMessage.success('项目勘察单生成成功！')
    loadHistory()
  } catch (error) {
    ElMessage.error('生成失败，请重试')
    console.error('生成勘察单失败：', error)
  } finally {
...
const loadHistory = async () => {
  ...
  try {
    const res = await getSurveyHistory()
