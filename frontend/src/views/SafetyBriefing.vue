<template>
  <div class="safety-briefing">
    <el-card shadow="hover" class="form-card">
      <template #header>
        <span class="section-title">安全交底</span>
      </template>
    ...
    <el-card shadow="hover" class="history-card">
      <template #header>
        <div class="history-header">
          <span class="section-title">安全交底生成历史</span>
...
import {
  ...
  getSafetyBriefingHistory,
  deleteSafetyBriefing,
  batchDeleteSafetyBriefing
} from '../api'
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(
      `确定要删除 "${row.project_name || '该工程'}" 的安全交底记录吗？`,
      '删除确认',
      {
  ...
  try {
    await deleteSafetyBriefing(row.id)
...
const handleBatchDelete = async () => {
  ...
  try {
    await batchDeleteSafetyBriefing(ids)
...
// 生成安全交底（不传任何选择项，后端自动从工程关联数据组合）
const handleGenerate = async () => {
  generating.value = true
  try {
    const res = await generateSafetyBriefing({
    ...
    ElMessage.success('安全交底生成成功！')
    loadHistory()
  } catch (error) {
    ElMessage.error('生成失败，请重试')
    console.error('生成安全交底失败：', error)
  } finally {
...
const loadHistory = async () => {
  ...
  try {
    const res = await getSafetyBriefingHistory()
