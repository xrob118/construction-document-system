<template>
  <div class="tech-briefing">
    ...
    <el-card shadow="hover" class="form-card">
      <template #header>
        <span class="section-title">技术交底</span>
      </template>
    ...
    <el-card shadow="hover" class="history-card">
      <template #header>
        <div class="history-header">
          <span class="section-title">技术交底生成历史</span>
...
import {
  getProjects,
  getWorkers,
  generateTechBriefing,
  getTechBriefingHistory,
  deleteTechBriefing,
  batchDeleteTechBriefing
} from '../api'
// 工程列表与选择
const projects = ref([])
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(
      `确定要删除 "${row.project_name || '该工程'}" 的技术交底记录吗？`,
      '删除确认',
      {
  ...
  try {
    await deleteTechBriefing(row.id)
...
const handleBatchDelete = async () => {
  ...
  try {
    await batchDeleteTechBriefing(ids)
...
const canGenerate = computed(() => {
  return selectedProjectId.value && selectedWorkers.value.length > 0 && selectedItems.value.length > 0
})
...
// 生成技术交底（不传任何选择项，后端自动从工程关联数据组合）
const handleGenerate = async () => {
  generating.value = true
  try {
    const res = await generateTechBriefing({
    ...
    ElMessage.success('技术交底生成成功！')
    loadHistory()
  } catch (error) {
    ElMessage.error('生成失败，请重试')
    console.error('生成技术交底失败：', error)
  } finally {
