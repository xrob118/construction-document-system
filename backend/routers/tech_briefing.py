...
class TechBriefingRequest(BaseModel):
    """技术交底生成请求体"""
    project_id: int
    worker_ids: List[int] = []
...
@router.post("/api/tech-briefing/generate", summary="生成技术交底文档")
def generate_tech_briefing(request: TechBriefingRequest):
    """生成技术交底文档
    ...
    try:
        ...
        data_dict = data_service.get_tech_briefing_data(
        ...
        file_path, pdf_path = generator.generate_tech_briefing(data_dict)
...
@router.delete("/api/tech-briefing/batch", summary="批量删除技术交底记录")
def batch_delete_tech_briefing(request: BatchDeleteRequest, db: Session = Depends(get_db)):
    """批量删除技术交底历史记录"""
    if not request.ids:
...
@router.delete("/api/tech-briefing/{doc_id}", summary="删除技术交底记录")
def delete_tech_briefing(doc_id: int, db: Session = Depends(get_db)):
    """删除技术交底历史记录"""
    record = db.query(GeneratedDoc).filter(GeneratedDoc.id == doc_id).first()
    if not record:
...
@router.get("/api/tech-briefing/history", summary="获取技术交底生成历史")
def get_tech_briefing_history(
    project_id: Optional[int] = None,
    db: Session = Depends(get_db),
):
    """获取技术交底的生成历史记录"""
    query = db.query(GeneratedDoc).filter(GeneratedDoc.doc_type == "tech_briefing")
    if project_id:
